...
import { type IWindowStore, WindowStore } from './store/window'
import { type IEngineWindow, EngineWindow, WindowState } from './window'

import { type IWindowsManager } from '../manager'

import { type IBridgeSurface } from '../bridge/surface'

import { Rect, RectDelta } from '../utils/rect'
import { overlap, wrapIndex } from '../utils/func'
...
export interface IEngine {
   ...
   */
  manage: (window: IEngineWindow) => void

  /**
   * Unregister the given window from WM.
   */
  unmanage: (window: IEngineWindow) => void
   ...
   */
  adjustLayout: (basis: IEngineWindow) => void
   ...
   */
  resizeFloat: (window: IEngineWindow, dir: CompassDirection, step: Step) => void
   ...
   */
  resizeTile: (basis: IEngineWindow, dir: CompassDirection, step: Step) => void
   ...
   */
  resizeWindow: (window: IEngineWindow, dir: CompassDirection, step: Step) => void
   ...
   */
  swapOrder: (window: IEngineWindow, step: Step) => void

  swapDirOrMoveFloat: (dir: Direction) => Promise<void>
   ...
   * visible than others.
   */
  setMaster: (window: IEngineWindow) => void

  /**
   * Toggle float mode of window.
   */
  toggleFloat: (window: IEngineWindow) => void
   ...
   */
  minimizeOthers: (window: IEngineWindow) => void
...
export class Engine implements IEngine {
  ...
  public windows: IWindowStore

  constructor (
    private readonly wm: IWindowsManager,
  ...
  public adjustLayout (basis: IEngineWindow): void {
    const surface = basis.surface
    const layout = this.layouts.getCurrentLayout(surface)
    const gaps = this.config.gaps

    if (layout.adjust) {
      const area = surface.workingArea.gap(
        gaps.screen.left,
        gaps.screen.right,
        gaps.screen.top,
        gaps.screen.bottom
      )
      const tiles = this.windows.visibleTiledWindowsOn(surface)
      layout.adjust(area, tiles, basis, basis.geometryDelta)
    }
  }

  public async resizeFloat (
    window: IEngineWindow,
    ...
    const hStepSize = surface.workingArea.width * 0.05
    const vStepSize = surface.workingArea.height * 0.05
    ...
    await window.forceSetGeometry(new Rect(geometry.x, geometry.y, width, height))
  }
  ...
  public async resizeWindow (
    ...
    if (EngineWindow.isFloatingState(state)) {
      await this.resizeFloat(window, dir, step)
    } else if (EngineWindow.isTiledState(state)) {
      this.resizeTile(window, dir, step)
    }
  }

  public arrange (): void {
    this.log.info('arrange')

    this.wm.screens.forEach((bridgeSurface: IBridgeSurface) => {
      this.arrangeScreen(bridgeSurface)
    })
  }
   ...
   */
  public arrangeScreen (screenSurface: IBridgeSurface): void {
    ...
    visibleWindows.forEach((win: IEngineWindow) => {
    ...
    if (this.config.maximizeSoleTile && tileableWindows.length === 1) {
      ...
      layout.apply(this.wm, tileableWindows, tilingArea)
    }
    ...
    visibleWindows.forEach((win: IEngineWindow) => { win.commit() })
    this.log.info(['arrangeScreen/finished', { screenSurface }])
  }

  public currentLayoutOnCurrentSurface (): WindowsLayout {
    return this.layouts.getCurrentLayout(this.wm.currentSurface)
  }

  public currentWindow (): IEngineWindow | null {
    return this.wm.currentWindow
  }

  public manage (window: IEngineWindow): void {
  ...
  public unmanage (window: IEngineWindow): void {
  ...
  public focusOrder (step: Step, includeHidden = false): void {
    const window = this.wm.currentWindow
    let windows

    if (includeHidden) {
      windows = this.windows.allWindowsOn(this.wm.currentSurface)
    } else {
      windows = this.windows.visibleWindowsOn(this.wm.currentSurface)
    }
    ...
    if (window === null) {
      this.wm.currentWindow = windows[0]
      return
    }
    ...
    this.wm.currentWindow = windows[newIndex]
  }
  ...
  public focusDir (dir: Direction): void {
    ...
    if (window === null) {
      const tiles = this.windows.visibleWindowsOn(
        this.wm.currentSurface
      )
      if (tiles.length > 0) {
        this.wm.currentWindow = tiles[0]
      }
      return
    }
    ...
    if (neighbor) {
      this.wm.currentWindow = neighbor
    }
  }

  public swapOrder (window: IEngineWindow, step: Step): void {
   ...
   */
  public async moveFloat (window: IEngineWindow, dir: Direction): Promise<void> {
    const surface = window.surface
    ...
    await window.forceSetGeometry(new Rect(x, y, geometry.width, geometry.height))
  }

  public async swapDirOrMoveFloat (dir: Direction): Promise<void> {
    const window = this.wm.currentWindow
    if (!window) {
      return
    }
    ...
    const state = window.state
    if (EngineWindow.isFloatingState(state)) {
      await this.moveFloat(window, dir)
    } else if (EngineWindow.isTiledState(state)) {
      this.swapDirection(dir)
    }
  }

  public toggleFloat (window: IEngineWindow): void {
    window.state = !window.tileable ? WindowState.Tiled : WindowState.Floating
  }

  public setMaster (window: IEngineWindow): void {
  ...
  public cycleLayout (step: Step): void {
    ...
    if (layout) {
      ...
      if (
        ...
        this.wm.currentWindow
      ) {
        this.minimizeOthers(this.wm.currentWindow)
      }
    }
  }
  ...
  public minimizeOthers (window: IEngineWindow): void {
   ...
   *
   * @param workingArea area in which we are allowed to work. @see IBridgeSurface#workingArea
