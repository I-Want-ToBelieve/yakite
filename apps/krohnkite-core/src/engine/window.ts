...
import { type IBridgeWindow } from '../bridge/window'
import { type IBridgeSurface } from '../bridge/surface'

import { type Rect, RectDelta } from '../utils/rect'
 ...
 */
export interface IEngineWindow {
  /**
   * Window unique id
   */
  readonly id: string
   ...
   */
  forceSetGeometry: (geometry: Rect) => Promise<void>
...
export class EngineWindow implements IEngineWindow {
  ...
  public readonly window: IBridgeWindow
  ...
  public get tileable (): boolean {
    return EngineWindow.isTileableState(this.state)
  }

  public get tiled (): boolean {
    return EngineWindow.isTiledState(this.state)
  }

  public get floating (): boolean {
    return EngineWindow.isFloatingState(this.state)
  }
  ...
  public set state (value: WindowState) {
    ...
    if (
      (winState === WindowState.Unmanaged ||
        EngineWindow.isTileableState(winState)) &&
      EngineWindow.isFloatingState(value)
    ) {
      this.shouldCommitFloat = true
    } else if (
      EngineWindow.isFloatingState(winState) &&
      EngineWindow.isTileableState(value)
    ) {
  ...
  public set surface (srf: IBridgeSurface) {
  ...
  constructor (window: IBridgeWindow, config: Config, private readonly log: Logger) {
    this.config = config

    this.id = String(window.id)
    this.window = window
  ...
  public async commit (): Promise<void> {
    ...
    // this.log.info(["Window#commit", { state: WindowState[state] }]);
    switch (state) {
      case WindowState.NativeMaximized:
        await this.window.commit(
      ...
      case WindowState.NativeFullscreen:
        await this.window.commit(undefined, undefined, undefined)
        break
      ...
      case WindowState.Maximized:
        await this.window.commit(this.geometry, true, false)
        break

      case WindowState.Tiled:
        await this.window.commit(this.geometry, this.config.noTileBorder, false)
        break
  ...
  public async forceSetGeometry (geometry: Rect): Promise<void> {
    await this.window.commit(geometry)
  }

  public visibleOn (srf: IBridgeSurface): boolean {
